...
export const replyMainComment = async(req, res) => {
  try {
    ...
    if(replyData.message.length < 1) return res.status(500).json({msg: "please type a comment!"})
...
export const editReplyMainComment = async(req, res) => {
  try {
    ...
    if(messageLength.length < 1) return res.status(500).json({msg: "please type a comment!"})
...
export const repliesToReplies = async(req, res) => {
  try {
    ...
    if(nestedRepliesData.message.length < 1) return res.status(500).json({msg: "please type a command"})
