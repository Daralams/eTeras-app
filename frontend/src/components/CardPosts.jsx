...
const CardPosts = ({ userId, posts }) => {
  ...
  const userLiked = async() => {
    ...
    const getUserIdFromLike = posts.flatMap(post => post.likes.map(like => like.userId));
  ...
  return (
    ...
    <div className="mt-3 flex justify-center">
      ...
      <div className="w-full border-2 rounded-md p-2 hover:shadow-lg" key={post.id}>
       ...
       <button className="text-[12px] flex justify-end bg-white px-2 py-1.5 border-[1px] rounded">Follow</button>
       </div>
      ...
      <div className="flex justify-between">
       <div className="p-1.5 w-11/12">
        <Link to={`/posts/${post.slug}`} className="fs-4 font-bold mb-2 hover:text-indigo-400">{post.title} <Link to={`/category/${post.category.slug}`} className="ml-3 text-[8px] mb-6 bg-indigo-300 py-0.5 px-1.5 text-white rounded border-[1px] border-indigo-500">{post.category.name}</Link></Link>
        
        {/*like & dislike btn*/}
       <div className="mt-2 flex gap-2">
         <button className={`text-xl ${liked ? 'text-red-500' : ''}`}
        onClick={() => likeDislike(post.id)}><SlHeart/></button>
