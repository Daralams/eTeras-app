...
const ChatHeader = ({ idInterlocutor, alternativeIdInterlocutor }) => {
  ...
  const getUsernameInterlocutor = async() => {
    const response = await axios.get(`http://localhost:3000/users/${idInterlocutor ? idInterlocutor : alternativeIdInterlocutor}`)
    setUsernameInterlocutor(response.data.data.username)
    ...
    <div className="bg-indigo-600 p-3 top-0 sticky w-full z-10">
      <div className="flex justify-between items-center">
        <div className="flex gap-2 items-center">
          ...
          <div className="p-2 flex justify-center items-center w-[40px] h-[40px] rounded-full bg-slate-100 font-bold">{usernameInterlocutor[0]}</div>
