...
const CommentsHistory = () => {
  ...
  const commentsHistory = async () => {
    try {
      ...
      const getTotalComment = response.data.posts;
      getTotalComment.map((totals) =>
        totals.comments.map(
          (comment) => console.log("CommentsL ", comment)
          // if(comment.user.id === userId) {
          //   setCommentTotal(comment.)
          // }
        )
      );
      console.log("comments total: ", getTotalComment);
    } catch (error) {
      // msh gabisa ngaff
